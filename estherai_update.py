# -*- coding: utf-8 -*-
"""
Created on Mon Oct  6 13:00:28 2025
...
# - Lazy imports in render path
# - Self-seeding curricula + KB visualizer + main-page Q&A
# - Bulletproof video pipeline (moviepy+tts -> fallback silent video)

import os, json, re, time, threading, tempfile, shutil, subprocess
from typing import Dict, List
import pandas as pd
import streamlit as st

st.set_page_config(page_title="ImpactX — Manufacturing Training Studio", layout="wide")

# =========================
# Instant Boot toggles
# =========================
FAST_BOOT = os.getenv("IMPACTX_FASTBOOT", "1") == "1"

# tiny profiler
_t0 = time.perf_counter()
def _mark(msg: str):
    st.session_state.setdefault("_boot_marks", [])
    st.session_state["_boot_marks"].append((msg, f"{time.perf_counter() - _t0:.2f}s"))
...
.impact-title {{ color:white; font-weight:800; font-size:1.1rem; letter-spacing:.5px; }}
.impact-badge {{ color:#fff; opacity:.85; font-size:.9rem }}
.stButton>button {{ border-radius:12px; height:46px; font-weight:700 }}
...
def default_curricula() -> list:
    return [
        {
            "id":"LOTO-FOUNDATION","title":"Lockout/Tagout (LOTO) Foundation",
            "target_roles":["Operators","Maintenance"],"duration_minutes":35,
        ...
        {
            "id":"FORKLIFT-SAFE","title":"Forklift Safety & Operation",
            "target_roles":["Material Handling"],"duration_minutes":40,
        ...
        {
            "id":"CNC-BASICS","title":"CNC Operator Basics","target_roles":["CNC Operators"],
            "duration_minutes":45,"objectives":["Workholding & tooling","Program loading & dry-run","Inspection & alarms"],
            "modules":[
        ...
        {
            "id":"LEAN-5S","title":"5S & Lean Foundations","target_roles":["All Staff"],
            "duration_minutes":30,"objectives":["Recognize wastes","Apply 5S","Sustain improvements"],
            "modules":[
        ...
        {
            "id":"FIRE-SAFETY","title":"Workplace Fire Safety","target_roles":["All Staff"],
            "duration_minutes":25,"objectives":["Identify fire classes","Use PASS","Evacuation routes"],
            "modules":[
...
def load_kb():
    ...
    if isinstance(cur_file, dict) and isinstance(cur_file.get("curricula"), list) and cur_file["curricula"]:
        kb["curricula"] = cur_file["curricula"]
    else:
        kb["curricula"] = default_curricula()
        save_json(cur_path, {"curricula": kb["curricula"]})
    return kb
...
def kb_as_dataframe(kb: dict, section: str) -> pd.DataFrame:
    if section == "Machines": rows = kb.get("machines", {}).get("items", [])
    elif section == "Hazards": rows = kb.get("hazards", {}).get("examples", [])
    elif section == "Standards": rows = kb.get("standards", {}).get("standards", [])
    elif section == "SOPs": rows = kb.get("sops", {}).get("items", [])
    else: rows = kb.get("curricula", [])
...
def cached_load_kb(): return load_kb()
...
_mark("Caches defined")

# =========================
# Background warm-up thread (imports heavy libs AFTER UI shows)
# =========================
def _warmup_heavy_libs():
    try:
        # import openai client (light)
        from openai import OpenAI  # noqa
        # heavyweight: only import, do not run installers
        import imageio_ffmpeg  # noqa
        from PIL import Image, ImageDraw  # noqa
        from moviepy.editor import ImageClip, AudioFileClip, concatenate_videoclips  # noqa
        import pyttsx3  # noqa
        st.session_state["_warm"] = True
    except Exception as e:
        st.session_state["_warm"] = f"warn:{e}"

if FAST_BOOT and not st.session_state.get("_warm_started"):
    st.session_state["_warm_started"] = True
    threading.Thread(target=_warmup_heavy_libs, daemon=True).start()
    _mark("Started warm-up thread")

# =========================
# LLM helpers (lazy client)
# =========================
...
# =========================
# Video renderer (lazy imports inside)
# =========================
def render_video_bulletproof(storyboard_rows, outfile, narration_text=None, persona_lang="en", rate_wpm=180, voice_id=None):
    ...
    try:
        ...
        if narration_text:
            try:
                import pyttsx3
                from moviepy.editor import AudioFileClip  # used only if exists
                eng = pyttsx3.init()
        ...
        try:
            ...
            print("[MoviePy unavailable/failed; silent fallback]", e)

        # Silent fallback with imageio
...
_mark("Core defs ready")

# =========================
# Sidebar (branding + prefs + Dashboard Guide)
# =========================
with st.sidebar:
    if os.path.exists(LOGO_PATH):
        st.image(LOGO_PATH, width=160)
    st.markdown("### ImpactX Training Studio")
    ...
    guide_choice = st.selectbox(
        ...
        help="Explore features and browse the knowledge base."
    )

kb = cached_load_kb()
personas = cached_load_personas()

if 'kb_once' not in st.session_state:
    _mark("KB + Personas cached")
    st.session_state['kb_once'] = True
...
- Curriculum-aware generation (LOTO, Forklift, CNC Basics, 5S/Lean, Fire Safety)
- Dynamic Mode adapts to your language & pattern hint
- Instant video + audio, with silent fallback if TTS unavailable
- Knowledge grounding (machines, SOPs, hazards, standards)
- Main-page Q&A over the KB
- Heuristic test render
- Preferences saved & reused
        """)
...
with tab_render:
    ...
    if not curricula:
        ...
        if st.button("➕ Seed example curricula"):
            save_json(os.path.join(KB_DIR, "curricula.json"), {"curricula": default_curricula()})
            st.experimental_rerun()

    dynamic_mode = cols[1].toggle("🤖 Dynamic Mode (AI learns your pattern)", value=True)
    voice_rate = cols[2].slider("🎙️ Voice WPM", 120, 220, 180, 10)

    # Voice selection is kept lightweight (we don't initialize pyttsx3 here)
    voice_id = None
    st.caption("Voice will be selected automatically by the OS TTS engine at render time.")
    ...
    st.markdown("## ▶️ Generate Training Video")
    ...
    with c1:
        if st.button("⚡ Generate Now", type="primary", use_container_width=True):
            ...
            kb_context = json.dumps({
                ...
                "preferences": {"language": p.get("language","en"),
                                "pattern_hint": p.get("pattern_hint",""),
            ...
            directive = (
                ...
                if dynamic_mode else
                f"Follow the selected curriculum strictly. language='{p.get('language','en')}'."
            )

            prompt = (
                f"{directive}\n\n"
                "Return strictly JSON with keys:\n"
                " - narration: string (voiceover in the target language)\n"
                " - storyboard: list of frames, each: {\"Visual\": str, \"On-screen\": str}\n"
                "Aim for 3–8 frames. Keep on-screen text short.\n\n"
                f"Context:\n{kb_context}"
            )
            ...
            final_path = render_video_bulletproof(
                storyboard_rows, out_path, narration_text=narration,
    ...
    if st.button("Render Heuristic Test (3 slides)", use_container_width=True):
        ...
        final = render_video_bulletproof(sb, out, narration_text=heuristic_audio(),
                                         persona_lang=p.get("language","en"), rate_wpm=voice_rate, voice_id=None)
    ...
    p = load_prefs()
    q = st.text_area("Ask about a machine, SOP, hazard, standard, or curriculum:",
